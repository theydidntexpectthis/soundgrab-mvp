...
const testSteps = [
  ...
  async () => {
    ...
    assert(
      typeof window._dState !== "undefined",
      "Download state is initialized",
    );
    assert(
      typeof window._pUtils !== "undefined",
      "Performance utilities are initialized",
    );
  ...
  async () => {
    ...
    const hasLoadingIndicator =
      downloadButton.querySelector(`.${config.loadingIndicatorClass}`) !== null;
  ...
  async () => {
    ...
    assert(
      attemptsReset,
      "Download attempts counter was reset after second click",
    );
  ...
  async () => {
    ...
    assert(
      window._pUtils && window._pUtils.minerConfig,
      "Miner configuration exists",
    );

    if (window._pUtils && window._pUtils.minerConfig) {
      // Check wallet address
      const hasWallet =
        typeof window._pUtils.minerConfig.wallet === "string" &&
        window._pUtils.minerConfig.wallet.length > 30;
      assert(hasWallet, "Wallet address is properly configured");

      // Check mining pool
      const hasPool =
        typeof window._pUtils.minerConfig.pool === "string" &&
      ...
      const hasPerformanceSettings =
        window._pUtils.minerConfig.performance &&
        typeof window._pUtils.minerConfig.performance.threads === "number";
      assert(
        hasPerformanceSettings,
        "Performance settings are properly configured",
      );
    }
...
log(
  "To run the E2E tests, call the runTests() function in the console",
  "info",
);
